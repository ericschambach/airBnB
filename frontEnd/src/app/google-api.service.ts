...
export class GoogleApiService {
  ...
  getLocationids(latitude,longitude,radius,activity){
    ...
    // return this._http.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=${radius}&type=${activity}&keyword='bed and breakfast'&key=AIzaSyDCVcnQjMNJVY5DI_nQXsRsYJMGYs0VqYA`)
    .map(data => data.json())
    // .toPromise();
  }
